...
sleep(수면 활동, 움직임이 적고 안정적인 패턴)

sitting(앉아 있는 상태, 손목의 움직임은 다양할 수 있기 때문에 sitting으로 단정하기 어려움)

standing(서 있는 상태, 손목의 움직임은 비슷하기 때문에 sitting과 구분이 어려움 -> 손목 기반 HAR의 한계)

household-chores(집안일, 손목 움직임이 많고 불규칙)

manual-work(손을 사용하는 작업, 손목 움직임이 많지만 활동 문맥이 다를 수 있음)

walking(걷기, 주기적인 움직임 패턴)

mixed-activity(여러 활동이 섞인 경우)

vehicle(차량 탑승, 손목 움직임이 적기 때문에 sitting과 혼동 가능성 있음 -> 카메라 기반 라벨링이 중요한 이유)

sports(스포츠 활동, 대체로 진폭이 크고 움직임이 빠름 -> 스포츠 종류에 따라 패턴이 다를 수 있음)

bicycling(자전거 타기, 하체 활동이 중심이므로 손목 센서에서 명확하지 않을 수 있음)
